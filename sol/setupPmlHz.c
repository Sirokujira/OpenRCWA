	...
	for (int k = kMin - lz + 1; k < kMax + lz; k++) {
		if ((i < 0) || (Nx <= i) ||
			...
			if      (mode == 1) {
				...
				id_t m = 0;
				if      (k  <  0) m = IHZ(i_,     j_,     0     );
				else if (Nz <  k) m = IHZ(i_,     j_,     Nz    );
				else if (i  <  0) m = IHZ(0,      j_,     k_    );
				else if (Nx <= i) m = IHZ(Nx - 1, j_,     k_    );
				else if (j  <  0) m = IHZ(i_,     0,      k_    );
				else if (Ny <= j) m = IHZ(i_,     Ny - 1, k_    );
				fPmlHz[num].m = m;
			}
			num++;
		}
